 ...
 *
 * Uses redirect-based login (popup is unreliable in many browsers).
 * Configure your own Entra ID app registration via VITE_FABRIC_CLIENT_ID.
 *
 * Setup:
 * 1. Register a SPA app in Entra ID (Azure AD)
 * 2. Add redirect URI: http://localhost:5173 (dev) + your production URL
 * 3. Add API permissions: Fabric API (Workspace.ReadWrite.All, Item.ReadWrite.All)
 * 4. Set VITE_FABRIC_CLIENT_ID in .env
 */
...
// App registration client ID — MUST be configured via env var.
// Without this, MSAL auth is disabled and the modal falls back to manual token entry.
const CLIENT_ID = import.meta.env.VITE_FABRIC_CLIENT_ID ?? '';
...
/**
 * Returns true if MSAL is configured (client ID provided via env var).
 */
 ...
 * Does NOT redirect mid-push — throws if consent is needed.
 */
