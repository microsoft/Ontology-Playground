...
## Phase 1 Alignment Workspace

- [x] Add a dedicated `/#/alignment` route for the review workflow shell
- [x] Add explicit alignment payload types and local mock queue fixtures
- [x] Add a separate Zustand alignment store instead of overloading app/designer state
- [x] Project the current ontology draft into a FastAPI-backed graph/schema/queue payload
- [ ] Replace the remaining mock fallback with a required live FastAPI-backed queue/schema/review flow
- [ ] Add staging write traceability and schema version guards to the live backend flow
