...
### Alignment Review Workspace

The ontology designer now includes an integrated `Review` tab for Phase 1
ontology alignment flows. Review one extracted candidate at a time, inspect
source evidence, apply schema mappings, and drive approve / defer / reject
actions against a published schema baseline without leaving the designer.
The review tab projects the current ontology draft through the alignment API
when `VITE_ALIGNMENT_API_BASE_URL` is set, turning user-authored entity types
and relationships into a live schema graph and review queue. When the env var
is absent, the workspace falls back to a local mock adapter so the UI remains
usable offline. The legacy `/#/alignment` route remains as a compatibility
deep link and opens the designer with the `Review` tab selected.
...
| `/#/designer` | Visual designer with Graph / RDF / Review tabs |
| `/#/designer/<source>/<slug>` | Designer with catalogue ontology (e.g. `/#/designer/official/cosmic-coffee`) |
| `/#/alignment` | Compatibility deep link that opens the designer on the Review tab |
...
| `VITE_ALIGNMENT_API_BASE_URL` | *(empty)* | Base URL for the FastAPI alignment backend used by the designer `Review` tab |
...
│   ├── store/            # Zustand stores (app state, designer state, alignment state)
