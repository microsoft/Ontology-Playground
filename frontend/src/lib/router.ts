 ...
 *   /#/alignment                                      → compatibility deep link that opens the designer review tab
 *   /#/review-graph                                   → full-page review and approved graph workspace
...
export function parseHash(hash: string): Route {
  ...
  if (segments[0] === 'alignment') {
    return { page: 'alignment' };
  }
  if (segments[0] === 'review-graph') {
    return { page: 'review-graph' };
  }
  if (segments[0] === 'learn') {
